...
def submit_job(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Link user to job
            job.user = user               # optional, could be the creator
            job.employer = user           # employer reference
            job.is_approved = False       # Await admin approval

            # Save any additional fields from form
            ...
            try:
                send_mail(subject_user, message_user, admin_email, [employer_email])
